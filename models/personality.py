...
def get_personality(user_id, name):
    return Personality.get_or_none((Personality.user_id == user_id) & (Personality.name == name))
...
class DeleteButton(Button):
    ...
    async def callback(self, interaction: Interaction):
        ...
        if delete_personality(self.row_id):
            await interaction.response.send_message("Personality deleted.", ephemeral=True)
            await interaction.message.delete()
        else:
            await interaction.response.send_message("I couldn't delete that personality.", ephemeral=True)